					...
					<div className='flex flex-wrap -m-4 justify-center'>
						{Object.keys(products).length === 0 && (
							<div className='font-bold text-3xl text-center'>
								Sorry All the sticker are out of stock. <p> New sticker are coming soon.</p>
								<p>Stay tooned.</p>
							</div>
						)}
...
export async function getServerSideProps(context) {
	...
	allStickers.forEach(item => {
		if (item.title in sticker) {
			if (!sticker[item.title].color.includes(item.color) && item.availQty) sticker[item.title].color.push(item.color);
			if (!sticker[item.title].size.includes(item.size) && item.availQty) sticker[item.title].size.push(item.size);
		} else {
